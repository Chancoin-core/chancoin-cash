...
chancoin:
	build/env.sh go run build/ci.go install ./cmd/chancoin
	@echo "Done building."
...
chancoin-linux-386:
	build/env.sh go run build/ci.go xgo -- --go=$(GO) --targets=linux/386 -v ./cmd/chancoin
	@echo "Linux 386 cross compilation done:"
	@ls -ld $(GOBIN)/chancoin-linux-* | grep 386

chancoin-linux-amd64:
	build/env.sh go run build/ci.go xgo -- --go=$(GO) --targets=linux/amd64 -v ./cmd/chancoin
	@echo "Linux amd64 cross compilation done:"
	@ls -ld $(GOBIN)/chancoin-linux-* | grep amd64

chancoin-linux-mips:
	build/env.sh go run build/ci.go xgo -- --go=$(GO) --targets=linux/mips --ldflags '-extldflags "-static"' -v ./cmd/chancoin
	@echo "Linux MIPS cross compilation done:"
	@ls -ld $(GOBIN)/chancoin-linux-* | grep mips

chancoin-linux-mipsle:
	build/env.sh go run build/ci.go xgo -- --go=$(GO) --targets=linux/mipsle --ldflags '-extldflags "-static"' -v ./cmd/chancoin
	@echo "Linux MIPSle cross compilation done:"
	@ls -ld $(GOBIN)/chancoin-linux-* | grep mipsle

chancoin-linux-mips64:
	build/env.sh go run build/ci.go xgo -- --go=$(GO) --targets=linux/mips64 --ldflags '-extldflags "-static"' -v ./cmd/chancoin
	@echo "Linux MIPS64 cross compilation done:"
	@ls -ld $(GOBIN)/chancoin-linux-* | grep mips64

chancoin-linux-mips64le:
	build/env.sh go run build/ci.go xgo -- --go=$(GO) --targets=linux/mips64le --ldflags '-extldflags "-static"' -v ./cmd/chancoin
...
chancoin-darwin-386:
	build/env.sh go run build/ci.go xgo -- --go=$(GO) --targets=darwin/386 -v ./cmd/chancoin
	@echo "Darwin 386 cross compilation done:"
	@ls -ld $(GOBIN)/chancoin-darwin-* | grep 386

chancoin-darwin-amd64:
	build/env.sh go run build/ci.go xgo -- --go=$(GO) --targets=darwin/amd64 -v ./cmd/chancoin
	@echo "Darwin amd64 cross compilation done:"
	@ls -ld $(GOBIN)/chancoin-darwin-* | grep amd64

chancoin-windows-amd64:
	build/env.sh go run build/ci.go xgo -- --go=$(GO) --targets=windows/amd64 -v ./cmd/chancoin
	@echo "Darwin amd64 cross compilation done:"
